...
A direct consequence for reading results: **`peak_import` reports the peak over the *considered* contributions only.** It can be much lower than the plan's largest tariff-eligible import - it can legitimately be `0.0` - while the same plan schedules substantial import at an excluded but still tariff-eligible timestep. When comparing runs, or checking a plan against a tariff, compare `P_grid_pos` over the tariff-eligible timesteps, not `peak_import` alone.
