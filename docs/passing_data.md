...
EMHASS can model a capacity / demand charge via the configuration parameter `capacity_cost_per_kw` (issue [#623](https://github.com/davidusb-geek/emhass/issues/623)). With `capacity_charge_interval_timesteps = 1` (default), it prices the highest eligible positive-import timestep; with `N > 1`, it prices the highest eligible completed tariff-interval average described below. `capacity_cost_per_kw` can also be a **list** of `K` independent rates to price `K` separately billed capacity/demand components in the same optimisation (e.g. two demand windows with different rates and incumbents) - see `docs/cookbook/tariff_demand_charge.md` Step 5 for the full contract; every `current_period_peak` / `capacity_charge_window` / `capacity_charge_current_interval_history` example below is the `K=1` form.
