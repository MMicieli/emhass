...
- `capacity_cost_per_kw`: Cost in currency per kW applied once to the modelled capacity/demand peak. `0` disables it. With `capacity_charge_interval_timesteps = 1`, the priced quantity is the highest eligible positive-import timestep; with `N > 1`, it is the highest eligible completed tariff-interval average. Set it to the marginal billing cost of increasing the applicable billed peak by 1 kW. Passing a **list** of `K` rates instead of a scalar prices `K` independent capacity/demand-charge components (e.g. two separately billed demand windows) in the same optimisation; see `docs/cookbook/tariff_demand_charge.md` Step 5 for the full multi-component contract, including how `capacity_charge_interval_timesteps`, `current_period_peak`, `capacity_charge_window` and `capacity_charge_current_interval_history` pair with a list here.
- `capacity_charge_interval_timesteps`: Number of native `optimization_time_step` intervals in one tariff demand-measurement interval. Default `1` preserves per-timestep capacity charging; use `N > 1` only when the tariff interval is exactly `N x optimization_time_step` (for example, 30-minute demand with a 5-minute step uses `6`). Invalid values warn and fall back to `1`. Rolling MPC can pass `capacity_charge_current_interval_history` for an interval already in progress; day-ahead/perfect runs have no elapsed-history input, so with `N > 1` their horizon start must align to a tariff interval boundary. With `capacity_cost_per_kw` set to a list of `K` rates, this may stay a scalar (broadcasts to every component) or become a list of exactly `K` entries for independent per-component measurement intervals.
