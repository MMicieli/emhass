...
- `sensor_power_battery`: The name of the signed AC-side battery power sensor in Watts from Home Assistant, for example 'sensor.power_battery'. Only used by battery self-identification (`set_use_battery_identification`); either sign convention works as it is auto-detected. This sensor is only retrieved when battery self-identification is enabled. With `number_of_batteries` greater than 1, provide a list of exactly `number_of_batteries` sensor names, one per battery, index-matched to the battery config lists. A single sensor name is never broadcast to more than one battery: one meter cannot tell two batteries apart, so without a full list identification skips with a warning saying what is missing.
...
- `def_current_operating_timesteps`: Per-load integer list: how many operating timesteps each must-run deferrable load has already completed today at the start of this optimization horizon. The optimizer schedules only the REMAINDER: `required_timesteps` and `target_energy` are each decremented by the elapsed amount, clamped at 0. For sequence loads, this means the load is currently running and is already this many timesteps into the sequence. This forces the optimizer to schedule the remainder of the sequence at window start. **Default `null` (absent key) or `[0, 0]` is an exact no-op** -- today's plan is unchanged. One non-negative integer per deferrable load, in the same order as `nominal_power_of_deferrable_loads`.
...
- `weather_forecast_pv_quantile_bias`: A blend factor that biases the PV forecast toward its conservative P10 (low) estimate. Type: float. Valid range: `[0, 1]`. Default: `0.0` (pure P50, unchanged behaviour). A value of `1.0` uses the P10 estimate exclusively. Intermediate values blend linearly: `estimate = bias * P10 + (1 - bias) * P50`. Higher values produce more defensive plans — the optimizer is told less solar will be available, so it tends to hold more battery reserve when solar is uncertain. Effective when `weather_forecast_method` is set to `solcast` (native Solcast P10), or when set to `list` and a `pv_power_forecast_p10` companion is also passed at runtime alongside `pv_power_forecast` (issue #1128); has no effect, and is ignored with a logged warning, for any other combination. See [docs/forecasts.md](forecasts.md#conservative-pv-bias-p10-blend) for the full concept and [docs/passing_data.md](passing_data.md#passing-an-optional-pv-p10-companion) for the external-forecast contract. A data-driven recommendation for this value can be obtained (report-only, never applied automatically) via the `pv-bias-calibration` action, documented in [docs/forecasts.md](forecasts.md#running-calibration-as-an-action-pv-bias-calibration-issue-1128).
...
- `set_battery_first_priority`: Set this to true to prefer draining stored battery energy before importing from the grid, so the battery is used up before any grid import while it is still above its minimum SoC. This is mainly useful on a flat (non time-of-use) tariff, where the solver would otherwise be free to interleave grid import with discharge while the battery is still full. Default is false. This is a soft penalty, not a hard constraint: importing while the battery is still charged is heavily penalized so drain-first wins on a flat tariff, but the optimizer can still import when that is the only feasible option (for example recharging to a target SoC with no PV, or a load that exceeds the battery's maximum discharge power), so it will not make the optimization infeasible. **With `number_of_batteries` greater than 1, the gate is on the FLEET, not per battery**: it compares the AGGREGATE stored energy across every battery against the AGGREGATE minimum energy (sum of `battery_minimum_state_of_charge * battery_nominal_energy_capacity` over all batteries), with a tolerance of 1% of the FLEET's total capacity (not any single battery's capacity). For a mixed fleet (batteries of very different sizes), a small battery can sit pinned at or near full charge while a large battery in the same fleet carries all of the drain, because the fleet as a whole is not yet considered "drained" - the gate does not force each battery to be drawn down individually. The 1% tolerance also scales with fleet size, so it can be materially larger, in absolute energy, than a small battery's entire usable SoC swing. For strict per-battery drain-first behaviour, run separate optimizations per battery instead of a single multi-battery plant.
...
With `number_of_batteries` greater than 1 (see the System configuration section below), `battery_soc_deficit_threshold`, `battery_soc_deficit_cost`, `battery_soc_surplus_threshold` and `battery_soc_surplus_cost` each accept either a single scalar (broadcast to every battery) or a list of exactly `number_of_batteries` entries, one per battery. A list of any other length is a configuration error and raises instead of being silently padded or truncated.

`weight_battery_charge` and `weight_battery_discharge` follow a different, nested rule once `number_of_batteries` is greater than 1, because a flat list is already meaningful today as a time-varying weight (see the note above these two parameters). The disambiguation is:
- A scalar broadcasts to every battery unchanged (each battery gets the same constant weight).
- A list of exactly `number_of_batteries` entries is read as one entry per battery, in battery order; each entry can itself be a scalar (constant weight for that battery) or a list (a time-varying weight for that battery, sliced to the horizon exactly like the single-battery case today).
- Any other list (one that is not exactly `number_of_batteries` long) is treated as a single shared time-varying series applied to every battery, preserving today's single-battery behaviour.
- Ambiguous case: if you want a *shared* time series and it happens to have exactly `number_of_batteries` points, it will be read as one point per battery instead (the per-battery rule takes priority). Nest it explicitly to get the shared-series reading: repeat it once per battery (`[series] * number_of_batteries`) or write out the same list under each battery's own entry.

With more than one battery, keeping both weights at their default of 0 is not recommended if your tariff can go negative. Consuming negatively priced energy is profitable to the optimizer, and with two or more batteries it can do so by having one battery charge while another discharges, a churn pattern a single battery physically cannot produce because of its charge/discharge exclusivity. A small non-zero `weight_battery_charge`/`weight_battery_discharge` prices that wear and suppresses the churn without affecting normal dispatch.
- `capacity_cost_per_kw`: A cost in currency per kW applied to the peak grid import power over the optimization horizon, for tariffs that include a capacity or demand charge. Defaults to 0, which disables the feature and leaves the plan unchanged. With a positive value the optimizer adds a single scalar term that prices the highest import power it plans, so it will flatten that peak where doing so is cheaper than the resulting change in energy cost. The peak is charged once because it is a power charge, not energy, so the term is not scaled by the timestep. With `capacity_charge_interval_timesteps = 1`, the priced quantity is the highest eligible positive-import timestep; with `N > 1`, it is the highest eligible completed tariff-interval average. Set it to the marginal value to you of shaving 1 kW off the applicable peak. Note this is a power-based capacity charge and is separate from `load_peak_hours_cost`, which is a time-of-use energy price. It also accepts a **list of two or more rates** (issue #540 Part B) to price K independent capacity/demand components — each with its own rate, measurement interval, eligibility window, MPC consideration and incumbent peak — in the same single optimization; every capacity runtime input (`current_period_peak`, `capacity_charge_window`, `capacity_charge_consideration`, `capacity_charge_current_interval_history`) then becomes a list of exactly K entries. A bare value, a one-element list `[x]` (what the config form serialises), or an empty list `[]` all normalise to the legacy single-component form (`[]` = disabled). See [passing_data.md](passing_data.md).
- `capacity_charge_interval_timesteps`: Number of native `optimization_time_step` intervals in one tariff demand-measurement interval. Default `1` preserves per-timestep capacity charging; use `N > 1` only when the tariff interval is exactly `N x optimization_time_step` (for example, 30-minute demand with a 5-minute step uses `6`). Invalid scalar values warn and fall back to `1`. Rolling MPC can pass `capacity_charge_current_interval_history` for an interval already in progress; day-ahead/perfect runs have no elapsed-history input, so with `N > 1` their horizon start must align to a tariff interval boundary. When `capacity_cost_per_kw` is a list of K rates, this may be a bare value (or `[N]`; both mean a shared measurement basis broadcast to every component) or a list of exactly K entries for independent per-component intervals — any other list length is a configuration error, not silently reinterpreted.
...
- `number_of_batteries`: The number of physical batteries in the plant. Defaults to 1, matching every EMHASS config written before this parameter existed - every existing config keeps working unchanged. Set it above 1 to model a fleet of distinct batteries (for example two different chemistries or sizes) as separate storage devices sharing the same PV/load balance, instead of a single lumped battery.
...
With `number_of_batteries` at its default of 1, all of the parameters above stay plain scalars, byte-identical to every config written before this parameter existed. With `number_of_batteries` greater than 1, each of `battery_discharge_power_max`, `battery_charge_power_max`, `battery_discharge_efficiency`, `battery_charge_efficiency`, `battery_nominal_energy_capacity`, `battery_minimum_state_of_charge`, `battery_maximum_state_of_charge`, `battery_target_state_of_charge` and `battery_stress_cost` (below) accepts either a single scalar, which is broadcast to every battery, or a list of exactly `number_of_batteries` entries, one per battery in order. A list of any other length is rejected with an error naming the parameter and the expected length - it is not padded or truncated, unlike the per-deferrable-load arrays above. `battery_target_state_of_charge` also becomes the per-battery fallback for `soc_init`/`soc_final` at runtime: `soc_final` is clamped per battery to that battery's own `[battery_minimum_state_of_charge, battery_maximum_state_of_charge]`, while `soc_init` is passed through unclamped (existing recovery behaviour), also per battery. `set_use_battery`, `set_nocharge_from_grid`, `set_nodischarge_to_grid`, `set_battery_first_priority`, `set_battery_dynamic`, `battery_dynamic_max`, `battery_dynamic_min` and `battery_stress_segments` stay global scalars that apply to every battery; they are not per-battery arrays.

The same shapes are accepted when overriding these parameters at runtime, and a runtime value always wins over the configured one. Note that this includes the broadcast: with `number_of_batteries` greater than 1, a runtime scalar replaces the configured value for every battery, even when the configuration held a per-battery list. So a payload that passes an aggregate value like `battery_charge_power_max: 4200` on a two-battery plant silently turns per-unit limits of `[2500, 2400]` into `[4200, 4200]`. When the masked configured list held genuinely distinct per-battery values, a warning is logged naming the parameter and both values. If you run more than one battery, pass runtime battery parameters as lists of `number_of_batteries` entries.

`battery_charge_power_derating` is per-battery too, recognised by shape: a list of `[soc_threshold, power_max]` pairs is one table used by every battery, a list of such tables is one per battery and must hold exactly `number_of_batteries` entries. Unlike the parameters above, an unusable value is logged as an error and ignored rather than rejected: charging then falls back to the flat `battery_charge_power_max`.

With `number_of_batteries` greater than 1, when two or more batteries are otherwise genuinely equivalent (identical cost/efficiency, so nothing real distinguishes them), a tiny internal tie-break decides which one the solver uses so the mathematical optimum is unique instead of an arbitrary LP/MILP choice. The tie-break penalizes total throughput (charge and discharge combined) scaled by battery index, so on a tie in EITHER direction the lowest-index battery is preferred, never the highest. It is always dominated by any genuine cost or efficiency difference between batteries - it only ever decides a truly degenerate choice, never overrides a real difference. This is an internal implementation detail (not a config parameter) documented here for completeness.

Making the optimum unique is not the same as guaranteeing the solver reports the same plan on every run: HiGHS is a MILP solver and stops once it is within `lp_solver_mip_rel_gap` of that optimum (default 0.01), which is several orders of magnitude larger than the tie-break's own contribution to the objective. Within that gap the solver is free to return any plan it likes, so strict run-to-run determinism needs a tight (or zero) `lp_solver_mip_rel_gap` - the same knob that governs general schedule repeatability, not something specific to multi-battery.